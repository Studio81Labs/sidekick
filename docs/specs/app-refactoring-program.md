...
jobs/history, benchmark, and training facades have been retired, and
architecture checks prevent production code from recreating or importing those
compatibility surfaces. Remaining endpoint facades, peer-feature exceptions,
backend compatibility modules, and final release audits are tracked in
...
| 12   | In progress | The frontend shared-type barrel and client/jobs/history/benchmark/training API facades are removed; endpoint facades, peer-feature allowlists, backend compatibility modules, release audits, and final docs remain |
