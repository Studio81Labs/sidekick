...
- The current
  ...
  economic and utility identities/configurations and carries them through the
  provider request boundary after fail-closed alignment validation. The scorer
  still uses `ev_bb`, and declarations or evidence pointers do not themselves
  prove ownership, input completeness, delivery rights, or that a retained
  source artifact actually implements the declared model.
