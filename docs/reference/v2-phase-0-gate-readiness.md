...
#409/#412. Resume source qualification when new authorized material arrives;
do not repeat public searches without a new lead or create speculative parser,
normalizer or lookup PRs. #409 needs missing action-origin semantics and the
representative corpus; #412 needs the actual export and its source/rights,
review, coverage and cost evidence; #414 remains dependent on both. The
historical ledger baseline does not override this checkpoint or those ADRs.
...
1. Assemble written rights evidence matching the actual source and delivery
   mode. Evaluate applicable published terms and key-specific conditions first;
   seek extra confirmation only for a material gap. The [source assessment](./v2-grading-reference-assessment.md#holdemresources-calculator-hrc)
   records the HRC evidence boundary.
