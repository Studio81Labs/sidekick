...
partial publication, so the runtime invalidates every browser session and
pending launch ticket and refuses new tickets until restart. Storage and export
requests already waiting behind the restore recheck authorization before
touching data. The player also clears its local credentials on an explicit
`503`, so no browser can resume ordinary work before process-start recovery.
